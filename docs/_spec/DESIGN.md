...
- **Topnav height:** 4rem, `position: relative` in document flow — the centre column of a three-column top row whose outer columns are the fixed sidebars' carousel headers (same height). Pinning pushes it horizontally via `margin-inline` rather than `transform`.
...
- **`topnav`** — glassmorphic, height 64px, in flow between the sidebar headers; scrolls away with the content. `body.scrolled` is toggled by a JS class so the sidebar rails can extend to the top (the `--nav-scrolled-bg` tokens are available for sites that want the bar itself to react).
